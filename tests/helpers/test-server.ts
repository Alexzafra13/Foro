...
export class TestServer {
  ...
  private setupMocks() {
    ...
    this.mockCommentRepository = {
      ...
      findByPostId: jest
        .fn()
        .mockImplementation(async (postId, pagination, userId) => {
          ...
          return {
            ...
            pagination: {
              ...
              totalPages: Math.ceil(
                comments.length / (pagination?.limit || 20)
              ),
              hasNext: false,
              hasPrev: false,
            },
      ...
      findReplies: jest
        .fn()
        .mockImplementation(async (parentCommentId, pagination, userId) => {
      ...
      findMany: jest.fn().mockImplementation(async (filters, pagination) => {
        ...
        if (filters?.postId) {
          comments = comments.filter(
            (comment) => comment.postId === filters.postId
          );
        }
      ...
      getCommentStats: jest.fn().mockImplementation(async (commentId) => {
        ...
        return {
          ...
          repliesCount: Array.from(this.comments.values()).filter(
      ...
      async (req, res) => {
        try {
          ...
          if (parentCommentId) {
            ...
            if (parentComment.postId !== postId) {
              return res.status(400).json({
                success: false,
                error:
                  "Parent comment must be a valid comment from the same post",
              });
            }
          ...
          res.status(201).json({
            ...
            message: newComment.parentCommentId
        ...
        try {
          ...
          if (sortBy === "voteScore") {
            comments.sort((a, b) => {
              if (sortOrder === "desc") {
                ...
                return (a.voteScore || 0) - (b.voteScore || 0);
              }
            });
          } else {
            comments.sort((a, b) => {
          ...
          const formattedComments = await Promise.all(
            paginatedComments.map(async (comment) => {
              ...
              const repliesCount = Array.from(this.comments.values()).filter(
          ...
          const totalComments = Array.from(this.comments.values()).filter(
